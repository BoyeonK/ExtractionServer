...
- **구매한 아이템은 창고(0~79)에만 놓을 수 있다** — `/api/items/purchase`의 `slot_index`가 창고 밖이면 `ERR_INVALID_SLOT`(400). 구매품이 정리 단계를 거치지 않고 곧바로 레이드에 나가거나 장착되는 것을 막기 위함이고, 창고가 만석이면 구매가 거부되므로 **클라이언트가 빈칸 없음을 먼저 막는 것과 짝이다**. 스냅샷 항목 쪽은 배치 통로라 위치를 자유롭게 두되 범위만 `0~107`로 막는다 — 세 영역 어디에도 없는 슬롯은 어느 조회 경로도 읽지 않는 유령 행이 된다. **`/match/start`의 CUSTOM 경로는 아직 이 상한이 없다** — 같은 형태의 스냅샷을 받지만 검사 강도가 다르니 한쪽을 보고 나머지를 짐작하지 말 것. 범위 상수의 출처는 `utils/slotLayout.js` 하나다.
