...
- **판매는 대상 슬롯의 스택 전체를 팔고, 영역을 가리지 않는다** — `/api/items/sell`의 스냅샷은 **판매 전** 상태라 대상 슬롯이 그 안에 있어야 하고(없으면 `ERR_SLOT_EMPTY`), 서버는 그 항목을 뺀 나머지를 써넣는다 — 구매의 `ERR_SLOT_OCCUPIED`와 정확히 반대 조건이다. 대상 슬롯 범위는 `0~107`로 창고 전용인 구매와 다르다(장착 중인 것도 팔 수 있다). **`quantity`는 부분 판매 지시가 아니라 주장이다 — 이걸 부분 판매 통로로 바꾸지 말 것**. 일부만 팔려면 클라이언트가 스택을 나눈 스냅샷을 보내면 되고, 수량 불변 조작이라 기존 구조가 그대로 지원한다. 지급액은 스냅샷의 수량에서 읽지만 총량 대조가 아이템별 합계를 고정하고 나머지가 다시 써지므로 가진 것보다 많이 받을 수 없다. **단가의 출처는 Redis `item_meta:<item_id>`의 `price`이고 이것을 채우는 쪽은 C++ 메인 프로세스다**(`RedisHandler::InitializeItemCache()`, 시동 시 1회) — Node 에서 `items` 를 직접 읽지 않는 유일한 자리이니 캐시가 비면 판매가 500 으로 실패하고, MySQL 가격을 고쳐도 메인 프로세스 재시동 전까지는 낡은 값이 쓰인다. 구매가와 판매가가 같은 `items.price` 라는 것도 현재의 확정 사항이다. 필드 구성은 `database/redis_keys.md` 4번.
- **판매 요청의 `item_id`·`quantity`는 스냅샷과 대조하는 주장이고, 이건 보안 장치가 아니라 클라이언트 버그 탐지기다** — 대상 슬롯의 스냅샷 항목과 하나라도 다르면 `ERR_ITEM_MISMATCH`(400). 악의적 클라이언트는 일관되게 거짓말하면 그만이라 이 검사로 막히지 않고, 그 방향은 총량 대조가 이미 막는다. 막는 것은 **클라이언트가 슬롯 계산을 틀려 엉뚱한 아이템이 조용히 팔리고 200이 나가는** 사고다 — 로그에도 정상 판매로 남아 사후 추적이 사실상 불가능한 종류라 400으로 갈라내는 값어치가 있다. 검사 자리는 트랜잭션 밖(`soldEntry` 조회 직후)이고, DB를 볼 이유가 없으니 안으로 옮기지 말 것. **본문 형식이 구매와 같아졌다고 검증을 공유하지 말 것** — 네 필드의 의미가 정반대다(구매는 지시, 판매는 주장). 구매의 검증 블록을 옮겨오면 슬롯 범위가 창고로 좁아져 인벤토리·로드아웃 판매가 죽고, `quantity`의 `99` 상한은 스택이 그 범위를 넘는 날 판매 불가로만 나타난다 — 상한은 스냅샷 항목과의 일치 검사가 이미 포함한다.
- **구매한 아이템은 창고(0~79)에만 놓을 수 있다** — `/api/items/purchase`의 `slot_index`가 창고 밖이면 `ERR_INVALID_SLOT`(400). 구매품이 정리 단계를 거치지 않고 곧바로 레이드에 나가거나 장착되는 것을 막기 위함이고, 창고가 만석이면 구매가 거부되므로 **클라이언트가 빈칸 없음을 먼저 막는 것과 짝이다**. 스냅샷 항목 쪽은 배치 통로라 위치를 자유롭게 두되 범위만 `0~107`로 막는다 — 세 영역 어디에도 없는 슬롯은 어느 조회 경로도 읽지 않는 유령 행이 된다. 스냅샷 검증은 세 라우트(`purchase`·`sell`·`match/start`)가 `utils/inventorySnapshot.js`를 함께 쓰므로 `0~107` 상한도 셋 다 같다. 범위 상수의 출처는 `utils/slotLayout.js` 하나다. **매치 흐름에만 걸 검사(반입 금지 아이템 등)가 생기면 공용 함수를 고치지 말고 `match.js` 쪽에 덧붙일 것** — 셋의 공통 규칙과 매치 고유 규칙이 한 함수에 섞이면 `items.js`가 매치 규칙을 조용히 물려받는다.
- **FREE 로드아웃은 인벤토리·장착 영역이 비어 있을 것을 요구하고, 그래서 FREE도 스냅샷을 받는다** — `/match/start`의 FREE가 스냅샷에서 `slot_index >= 80` 항목을 하나라도 보면 `ERR_LOADOUT_NOT_EMPTY`(400). 순수한 스냅샷 검사라 트랜잭션 앞에 두며 DB를 보지 않는다. 조건을 세운 이유는 레이드 종료의 `NotifyPlayerLeftRequest::ApplyInventoryToDb()`가 **로드아웃 타입도 이탈 사유도 보지 않고** `slot_index >= 80`을 지우기 때문이다 — 전에는 FREE가 스냅샷 영속(`/match/start`)도 `DELETE`(`/match/connect`)도 건너뛰어, 귀환 전리품을 창고로 옮긴 클라이언트의 로컬 재배치가 DB에 닿지 못한 채 다음 레이드 종료에 지워졌다. **FREE의 스냅샷 영속을 걷어내면 조건이 클라이언트가 만족시킬 수 없는 것이 된다** — 배치를 DB에 반영하는 경로가 그 셋뿐이라 유저가 스스로 인벤토리 영역을 비울 수단이 사라진다. 창고가 만석이면 FREE도 거부되는데 정상 경로는 판매로 덜어내는 것이고 예외 처리를 두지 않는다. `/match/connect`의 `DELETE`는 CUSTOM 조건을 유지한다 — FREE 쪽은 이미 비어 있어 무의미하다. **이 조건이 막지 않는 것 하나** — 귀환 시 프리셋 장비(무기 2·방어구·탄약)가 `user_inventory`에 그대로 INSERT돼 영구 소유가 되는 것은 별개 결정이다.
