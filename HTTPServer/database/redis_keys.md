...
2. `active_match:<db_id>` 의 해제
    ...
    한 판 10분 미만 확정을 전제로 5분 여유를 둔 값이고, 이 10분은 코드가 강제한다 —
    `GameRoom::ROOM_LIFETIME_MS`(600초)가 룸 수명을 상한 짓고 만료는 전원 사망으로 처리된다.
    `ROOM_LIFETIME_MS < ACTIVE_MATCH_TTL_SEC × 1000` 은 두 프로세스를 가로질러 `static_assert`
    로 묶을 수단이 없는 불변식이다 — `src/CLAUDE.md` 의 락 항목 참조.
