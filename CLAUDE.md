...
- **통보의 대상·시점·보장 범위를 바꾸면 `.proto`의 설명 주석을 함께 고칠 것** — `.proto`에는 프로토콜 자체의 설명(필드 의미, 거부 사유 목록, 통보 대상과 보장 범위)만 남긴다. 이 주석은 서버 빌드에 영향이 없어 틀려도 아무것도 깨지지 않고, 깨지는 자리가 클라이언트라 발견이 늦다. 구현 diff에 `.proto`가 없으면 주석을 안 봤다는 뜻으로 읽는다.
- **`External_Protocol.proto`의 프로토콜이 바뀌면 `LATEST_VERSION`의 뒷 숫자를 올릴 것** — `HTTPServer/index.js`. 주석만 고친 변경은 제외하고, 그 밖에는 **필요한지 판단하지 않는다** — 새 message·새 필드처럼 구 클라이언트가 무시해도 되는 변경까지 전부 올린다. 과하게 튕기는 비용(클라이언트 재다운로드)이 어긋난 채 도는 비용보다 싸고, 판단을 끼우면 그 판단이 틀리는 날 조용히 깨진다. 사용자가 명시적으로 지시할 때도 올린다. 짝이 어긋나도 서버 빌드·통신은 전부 정상이며, 낡은 클라이언트가 잘못 파싱한 패킷이 그대로 서버 로직에 들어오는 형태로만 드러난다. 형식은 `<라벨>-<정수>`이고 클라이언트는 **문자열 완전 일치**로 비교하므로 자릿수가 늘어도 문제없다(대소 비교가 아니다). 라벨이 바뀌면(`alpha` → `beta`) 숫자는 1부터 다시 시작한다.
...
- **외부 라이브러리**: `abseil-cpp` (FetchContent)
