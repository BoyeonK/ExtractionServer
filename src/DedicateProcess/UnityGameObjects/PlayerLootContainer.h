...
class PlayerLootContainer : public Container {
public:
    PlayerLootContainer(uint32_t objectId, const Vector3& position, float yawAngle,
                        const PlayerInventory& inventory)
    : Container(objectId, ObjectType::PlayerLoot, true, position) {
...
private:
    ...
    static_assert(static_cast<uint32_t>(PlayerInventory::INVENTORY_SLOT_COUNT) + 5 <= DEFAULT_CONTAINER_VOLUME,
                  "플레이어 전리품 컨테이너 용량 부족 - PlaceItem() 은 범위를 넘으면 아이템을 조용히 버린다");
