...
- **응답 없는 세션의 재전송 큐는 이탈 처리로만 정리된다** — 개별 제거는 `ProcessIncomingAck()`, 일괄 정리는 `ProcessLeaves()`의 사전 패스와 `FinalizeLeave()`뿐. **사전 패스를 분리 루프 안으로 되돌리지 말 것** — `DetachPlayer()`가 방송하는 사망·킬피드·디스폰은 같은 틱에 분리되는 **다른** 세션의 큐에도 들어가는데, 루프 안에서 지우면 뒤늦게 분리되는 세션이 그것들을 재전송 없이 잃는다(`keepSeq`가 한 장만 남기고, AllKill·끊김 경로는 그마저 0이라 전부 지운다). 최초 전송은 이미 나갔으므로 증상은 사망 유예 5초 동안 SPECTATING 세션에 남의 시신이 서 있는 것뿐이지만, 유실 확률이 가장 높은 구간이 하필 이 패킷 몰림이다. 살아있는 세션에는 무한 재전송하므로 큐의 상한을 실질적으로 정하는 것은 끊김 감지 6초다 — 임계값을 늘리면 죽은 세션의 큐가 그만큼 오래 남아 `GetRetransmitCandidates()`가 매번 전체를 순회한다.
...
- **플레이어 최대 HP 는 200 으로 확정됐다** — `PlayerObject::DEFAULT_MAX_HP`(100배 스케일이라 `20000`). 아이템 수치와 달리 이 값은 DB·생성 스크립트 밖이라 클라이언트와 **손으로** 맞춘다(루트 CLAUDE.md 「데이터 원천 규율」의 자동화 밖 구간 셋 중 하나). 어긋나도 컴파일·통신은 정상이고 체력바 표시만 조용히 틀어진다.
...
- **룸 수명은 생성 시각으로부터 10분이고 만료 처리는 전원 사망이다** — `GameRoom::ROOM_LIFETIME_MS`. 기산점이 첫 입장이 아니라 룸 생성이라 클라이언트 로딩 시간이 플레이 시간에서 깎이고, 대신 아무도 접속하지 않은 룸까지 상한이 덮는다. 검사 지점은 `ProcessLeaves()` 맨 위 하나다 — `Update()`에 두지 않은 이유는 그쪽이 "override 하면 마지막에 베이스를 부를 것" 규율에 기대는 자리라 규율이 깨진 파생 룸만 조용히 상한을 잃기 때문이고, 맨 위에 둬야 `AllKill()`이 찍은 마킹이 같은 틱의 이탈 루프에서 소비된다. `AllKill()`은 새 파괴 경로가 아니라 전 세션에 `MarkLeaving(DEAD)`를 찍는 루프이고 뒤는 기존 파이프라인(`DetachPlayer` → 유예 5초 → `FinalizeLeave` → `CheckAllLeft` → `ReserveRoomDestroy`)이 그대로 처리한다 — 만료부터 룸 회수까지 약 6초, 락 해제는 그보다 앞선 `DetachPlayer` 시점이다. 딸린 사실 다섯:
  ...
  - **만료 사망에는 가해자가 없고, 클라이언트는 일반 사망과 똑같이 취급하기로 확정됐다** — `killer_object_id`는 `NO_ATTACKER`, 이름은 빈 문자열, `DespawnReason`은 `DESPAWN_DEAD`. 사망 화면에서 「시간 초과」를 구분해 띄우지 않으므로 통보를 가를 이유가 없다 — **만료 전용 통보나 `DespawnReason` 신설을 제안하지 말 것.**
  - **만료 직후 사망 통보가 세션당 100ms~1초 간격으로 재전송되는 것은 정상이다** — 사망 화면의 클라이언트가 상향을 끊으면 ack 가 없어 pending 이 남고 `DediServerService::CheckRetransmits()`가 계속 보낸다(간격은 `clamp(rtt×1.5, 50ms, 1000ms)`, 스케줄러가 세션당 100ms 로 훑는다). 상한은 클라이언트 행동에 의존하지 않는다 — `IsFinalizeReady()`가 DEAD 만은 ack 를 보지 않고 `DEATH_GRACE_MS`(5초)만 보므로 `FinalizeLeave()`가 pending 을 비우고, 재전송 루프는 FINALIZED 세션을 건너뛴다. **SPECTATING 세션의 재전송을 억제하지 말 것** — ack 가 없는 이상 유실로부터 사망·디스폰 통보를 지키는 수단이 재전송뿐이고, 유실 확률이 가장 높은 구간이 하필 이 패킷 몰림이다. 비용은 인원수의 제곱으로 붙으므로(세션당 최대 2(N−1)장 × 5초) 룸 정원이나 `DEATH_GRACE_MS`를 늘릴 때 위 33장 항목과 같이 볼 것.
...
- **파생 룸의 주기 동작은 전부 같고, 그것이 현재의 의도다** — `TestGameRoom`·`TenerifeGameRoom` 어느 쪽도 `Update()`를 override하지 않는다. 룸마다 다르게 동작시킬 계획은 없으며 `virtual`은 여지로만 남긴 것이다 — 테네리페에 뭔가 빠졌다고 읽지 말 것. 갈리는 것은 생성자뿐이다(테스트 룸만 `InitTestGameRoom()`으로 `TestItemBox`를 정적 스폰한다).
...
- **월드 배치 아이템의 `instanceUid`는 `WORLD_ITEM_UID_BASE`(`Items.h`, `1<<62`)에서 시작하는 룸 로컬 단조 카운터다** — 서버는 uid를 **읽는 곳이 하나도 없다**(`Container.cpp`·`PlayerInventory.cpp`의 직렬화 두 곳에서 내보내기만 하고, `ApplyInventoryToDb()`도 무시하고 새 행을 INSERT한다). 따라서 요구는 하나뿐이다 — **클라이언트가 uid를 키로 써도 룸 안에서 겹치지 않을 것.** 상위 비트를 쓰는 이유는 같은 룸의 플레이어 전리품 컨테이너가 실 DB uid(BIGINT AUTO_INCREMENT)를 그대로 싣기 때문이고, `TestItemBox`도 같은 체계를 쓴다(옛 1001~1003은 DB uid와 겹칠 여지가 있었다). **DB uid를 이 공간으로 올리지 말 것** — 그 순간 두 공간이 다시 만난다.
...
- **aggro 변경(`C2DRequestNpcAggro`)과 주도권 이전(`C2DRequestNpcAuthority`)은 별도 패킷이고 합치면 안 된다** — 전자의 관문은 `발신자 == _targetId`, 후자는 `클램프(claimed) > _aggro`다. 「주도권자가 아닌데 더 높은 값을 주장하니 겸사 넘겨주자」로 전자를 탈취 경로로 흘려보내면, 반납 이후 도착한 낡은 감쇠 요청이 재탈취를 일으켜 **서버만 주도권자가 있다고 믿는 정지 상태**가 된다(reliable C2D는 순서를 보장하지 않으므로 역전은 정상 플레이에서 나온다). 분리 덕에 낡은 감쇠 요청은 관문에서 그냥 버려진다. **피격 경로(`ApplyDamageAuthority()`)는 이 경고에 걸리지 않는다** — 후자의 관문을 뚫는 것이 아니라 그대로 지나가기 때문이다(아래 항목).
...
- **`aggro`가 MAX면 아무도 탈취할 수 없고 그것이 의도다** — 비교가 `>`라서다. 감쇠가 클라이언트 몫이므로 교착이 아니고, 넘기려면 현 주도권자가 먼저 내려야 한다. 등급을 나눠 쓰는 것(발걸음은 낮게, 피격은 MAX)은 **클라이언트 사이의 규약일 뿐** 서버가 강제하는 것이 아니며, 자기 이익만 좇는 클라이언트는 언제나 MAX를 부른다 — 감내하기로 확정.
- **피격은 `ApplyAuthorityRequest(가해자, MAX)`와 같은 연산이고, 그래서 새 관문이 없다** — `ApplyDamageAuthority()`는 그 위임 한 줄이다. `클램프(MAX) <= _aggro`가 `_aggro == MAX`와 동치라, 「aggro가 MAX 미만이면 가해자에게 넘어가고 MAX면 무시」가 기존 관문에서 그대로 떨어진다(주인 없는 경우는 `_aggro == MIN`이라 자동 포함). **피격에 별도 관문을 파지 말 것** — 관문이 갈리는 순간 위 항목이 경고한 실패 모드가 되살아난다. 겸사 왕복도 구조적으로 막힌다(한 번 MAX가 되면 뒤따르는 피격이 전부 거부되므로 번갈아 쏴도 주도권이 튀지 않는다). 부여 지점은 `Handle_C2D_RequestWeaponFire`이고 **`IsDeathPending()` 분기의 else라 죽은 NPC에는 부여되지 않는다** — 순서를 뒤집으면 `D2CNotifyNpcAuthority` 뒤에 `D2CNotifyObjectKilled`가 붙어 reliable 한 장이 통째로 낭비된다. `TakeDamage()` 안에 넣을 수 없는 이유는 `OnDeath()`와 같다(`GameRoom&`가 없다). 주도권자 본인의 명중은 aggro만 MAX로 올리고 통보하지 않는다 — 클라이언트의 aggro 사본이 그동안 낡지만 다음 요청이 절대 지정이라 누적되지 않는다.
- **aggro 값의 의미는 클라이언트가 정하고 서버는 경위를 보지 않는다** — `C2DRequestNpcAuthority.aggro`는 사유가 아니라 값이다. Reason enum을 두고 서버가 값을 매기는 안은 **서버가 그 사유를 검증할 상태를 갖고 있지 않기 때문에** 채택하지 않았다 — 검증하려면 NPC 인지·이동 연산을 서버가 떠안아야 하는데 그건 「AI는 서버에서 다루지 않는다」가 배제한 것이고, 연산을 넘긴 이상 산출된 결과를 그대로 받는 것이 일관되며, Reason은 권한이 아니라 장부만 늘린다. **재검토 조건은 서버가 NPC 인지·이동 연산을 떠안기로 방침이 바뀌는 때 하나다.** 수용하는 비용 하나 — 클라이언트 측 등급 표 변경은 `.proto`를 건드리지 않아 `LATEST_VERSION`이 오르지 않으므로, 서로 다른 표를 든 클라이언트 버전이 공존할 수 있고 같은 상황에서 다른 값이 나가도 서버 동작도 로그도 전부 정상이다.
- **MIN을 바꾸면 클라이언트의 반납 값이 짝이다** — MIN 지정이 곧 반납인데, 클라이언트가 MIN의 숫자를 박아두면 MIN이 내려간 뒤 그 값이 반납이 아니라 살아 있는 주장이 된다(4 → 1로 내렸을 때 옛 4는 aggro 4다). `.proto`에 「0을 보내면 클램프가 처리한다」를 적어 클라이언트가 MIN 값을 몰라도 되게 했으므로, 그 규약을 지우지 말 것.
- **주도권 통보(`D2CNotifyNpcAuthority`)는 주도권이 실제로 옮겨갈 때만 발행한다** — aggro 중간 변경마다 reliable 브로드캐스트를 내면 in-flight 33장 예산을 NPC가 통째로 태운다. `HostileNPC::RequestResult`의 `AUTHORITY_CHANGED`가 그 판단자다. 발행 계기는 둘(요청 승인, 피격)이고 **후자는 요청 없이 도착한다.** **클라이언트 쪽 짝 셋이 `.proto` 주석에 있다** — 절대값이므로 `rSeqNum`으로 낡은 통보를 버릴 것, 「내가 주도권자인가」의 출처는 이 통보뿐이라 반납 요청 후 로컬에서 먼저 놓지 말 것(먼저 놓으면 재탈취 시 서버와 인식이 갈린 채 굳는다), 그리고 요청 없이 받은 통보도 즉시 `npc_states` 송신 시작으로 받아들일 것. **파괴 시에는 해제 통보가 나가지 않는다** — `D2CNotifyObjectKilled`의 `victim_object_id`가 자기가 쥔 NPC면 주도권도 함께 놓는 것이 클라이언트 몫이다(서버는 오브젝트가 사라지므로 할 일이 없다).
