...
enum : uint16_t {
	...
	PKT_ID_M2D_UPDATE_ENTRY_TOKEN = 102,
	PKT_ID_D2M_NOTIFY_PLAYER_LEFT = 103,
	PKT_ID_D2M_NOTIFY_ROOM_DESTROYED = 104,
};
...
public:
	static void Init() {
		...
		GProtoPacketHandler[PKT_ID_D2M_NOTIFY_PLAYER_LEFT] = [](Session* pSession, unsigned char* buffer, int32_t len) { return HandlePacket<IPC_Protocol::D2MNotifyPlayerLeft>(Handle_D2M_NotifyPlayerLeft, pSession, buffer, len); };
		GProtoPacketHandler[PKT_ID_D2M_NOTIFY_ROOM_DESTROYED] = [](Session* pSession, unsigned char* buffer, int32_t len) { return HandlePacket<IPC_Protocol::D2MNotifyRoomDestroyed>(Handle_D2M_NotifyRoomDestroyed, pSession, buffer, len); };
	}
	...
	static SendBuffer* MakeSendBuffer(const IPC_Protocol::D2MNotifyPlayerLeft& pkt) { return MakeSendBuffer(pkt, PKT_ID_D2M_NOTIFY_PLAYER_LEFT); }
	static SendBuffer* MakeSendBuffer(const IPC_Protocol::D2MNotifyRoomDestroyed& pkt) { return MakeSendBuffer(pkt, PKT_ID_D2M_NOTIFY_ROOM_DESTROYED); }
