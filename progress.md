...
- [x] (2026-07-17 #0) CombatObject 레이어 추가 — `UnityGameObject` → `CombatObject` → `PlayerObject` 계층 도입. HP(maxHp, currentHp), Shield/AP(maxShield, currentShield, damageReductionRate, shieldRegenPerSec) 데이터 필드 추가. 데미지 처리 로직은 아머 시스템 확정 후 구현 예정 (`UnityGameObjects/CombatObject.h`, `PlayerObject.h`, `CMakeLists.txt`)
...
### 진행 우선사항
1. **데미지 처리 구현** — 아머 시스템 확정 후 `CombatObject::TakeDamage()` 구현 + `Handle_C2D_RequestWeaponFire()` 피격 스텁 연결. 비플레이어 전투 오브젝트는 `FindNonplayerObject()` → `dynamic_cast<CombatObject*>`로 타입 판별 후 처리. 사망 처리(브로드캐스트, 세션 정리, 리스폰) 포함
