...
2. **Turret 수치 확정과 배치** — 서버 구현은 끝났으나(2026-09-18 #0) 스폰하는 코드가 없어 인스턴스가 0이고, 그래서 피격→사망→회수→통보 경로는 **여전히 한 번도 실행된 적이 없다**. 필요한 것 둘 — ① `HostileNPCs.h`의 잠정 수치 확정(`DEFAULT_MAX_HP` 30000·`ATTACK_DAMAGE` 1200은 AK-47 `baseDamage` 4800과 플레이어 200HP만 보고 넣은 값이고, MIN/MAX_AGGRO는 기본값 4/8을 그대로 쓴다) ② 어느 맵에 몇 대를 어디에 놓을지 — 테네리페 컨테이너 64대처럼 `MapDataManager`의 맵별 정적 테이블이 자리로 유력하다. 스폰이 붙는 순간이 곧 전 경로의 첫 실행이므로 그 자리에서 디버그할 것
...
- **비플레이어 사망 경로는 아직 한 번도 실행되지 않는다** — `HostileNPC`와 `Turret`은 생겼지만 스폰하는 코드가 없어 `FindNonplayerObject()`가 전투 오브젝트를 찾는 일이 여전히 없다. 검증용 더미를 만들지 않기로 했으므로 Turret 배치가 붙는 시점에 피격→사망→회수→통보 전체를 그 자리에서 디버그할 것. 그때 지켜야 할 불변식 둘 — ① 회수 지시는 `TakeDamage()` 안이 아니라 **호출부가** 한다(안에서 `delete`하면 호출부가 직후에 읽는 `GetCurrentHp()`가 use-after-free) ② `OnDeathResolved()`에 `_dynamicObjects`의 반복자를 넘기지 않는다(훅이 흔적을 스폰하면 rehash로 무효화)
- **`ObjectType::Turret`(9)의 클라이언트 프리팹 매핑이 아직 없다** — `object_type`이 `int32`라 `.proto`는 그대로이고 서버 빌드·통신·로그도 전부 정상이라, 클라이언트가 스폰 통보를 받고도 오브젝트를 만들지 못하는 형태로만 드러난다. `D2CNotifyNpcAuthority`·`D2CUpdateNpcStates`·`D2CBroadcastNpcAttack` 셋과 `C2DUpdatePlayerState.npc_states` 필드도 클라이언트 구현이 선행돼야 Turret이 실제로 움직인다 — **배치를 붙이기 전에 상위 세션에 클라이언트 동반 수정을 요청할 것**
...
- **2026-09-18 #0의 변경도 아직 컴파일된 적이 없다** — `DistributeLoot()`과 같은 사정으로, 확인한 것은 `PktId` 짝 전수 대조(`enum.h` ↔ `.proto` ↔ 등록·Make 함수 6개)와 필드명·기존 관용구 일치까지다. 문법·타입은 다음 Linux 빌드에서 처음 확인된다
