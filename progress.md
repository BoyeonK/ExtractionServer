...
1. **시신 컨테이너 및 사망 연출** — 사망 시 이탈 처리(오브젝트 제거·퇴장 통보·인벤토리 소실)는 구현 완료. 남은 것 셋
    - **시신 컨테이너** — `GameRoom::DetachPlayer()`의 TODO 자리(오브젝트 제거 **직전**, `position`과 인벤토리를 아직 쥐고 있는 유일한 지점)에서 `Container` 파생 오브젝트를 스폰하고 `Clear()` 대신 인벤토리를 그쪽으로 '이동'시킨다. `Container::PlaceItem()`이 protected이므로 `PlayerInventory`를 통째로 받아 채우는 전용 파생 클래스가 낫다. **런타임 스폰이라 할 일 8번(오브젝트 생성 브로드캐스트)에 의존** — 단독으로 구현하면 클라이언트에 보이지 않는다
...
8. **`GameRoom`의 오브젝트 생성 브로드캐스트 6곳** — `SpawnStaticObject`/`SpawnDynamicObject`/`SpawnPlayerObject`(Test·Winchester 각각)의 `TODO : 생성 정보를 broadcast`. 전송 수단은 준비됐으나 그대로 꽂으면 안 된다 — ① 정적 오브젝트는 `TestGameRoom` 생성자(`InitTestGameRoom()`)에서 스폰되어 그 시점엔 세션이 0명이고 청사진 응답으로 이미 전달되므로, 브로드캐스트가 의미 있는 건 런타임 스폰뿐이다 ② reliable로 N개를 개별 통보하면 ACK 윈도우(33개) 한계에 걸리므로 `FillStaticObjects()`의 청킹 패턴을 따라 묶어야 한다 (`GameRoom.cpp`)
    - **할 일 1번(시신 컨테이너)이 여기에 걸려 있다** — 시신은 게임 도중 생기는 첫 런타임 스폰이다
...
- **reliable in-flight 33개 초과 시 영구 ACK 불가** — ACK 커버리지는 `ackRSeqNum` 1개 + 비트필드 32개 = 총 33개다(`PlayerSession.cpp:85`, `120-130`). `_pendingReliable`에 34개 이상이 동시에 쌓이면 가장 오래된 것은 클라이언트가 이미 받았어도 알릴 수단이 없어 MAX_RETRY까지 재전송된다(수신 측은 `UpdateRRecvState()`에서 `diff > 32`로 버리므로 정확성은 유지, 대역폭만 낭비). **첫 reliable 브로드캐스트인 `D2CDespawnPlayerObject`가 들어왔다** — 이탈 1건당 1장이고 룸 인원이 4명이라 아직 여유가 크지만, 이 경로가 늘어나면 도달 가능해진다. 대량 통보는 개별 전송 대신 청킹으로 묶을 것. 근본 해결은 비트필드 64비트 확장 또는 송신 창 제한이며 별도 작업
