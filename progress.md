...
2. **클라이언트 연동 — 구현 확인 완료, 실환경 검증과 부분구현 둘만 남았다** — 2026-08-29 클라이언트 코드 실사 결과, 이 항목이 들고 있던 미구현 목록(스폰 통보 36·선제 `D2CSpawnPlayerObject` 13, 무기 교체 37/38, 재장전 42/43, 재장전 연출 44/45, 킬 피드 40, 오브젝트 킬·디스폰 41/39, 헤더 `timestampEcho` 에코, 탄약 차감, 장착 시 손 슬롯 이동 규칙, 자기 실드 재생 예측, 사망 유예 처리)은 **전부 구현돼 있어 목록에서 제거했다**. 남은 것 둘 —
    ...
    - **탄창 언로드 규칙 미러 + 검산 미구현** — 무기 슬롯 장착·해제 시 서버가 탄창을 인벤토리로 쏟는 규칙(`C2DRequestEquipItem` 주석에 명문화)을 클라이언트가 같이 구현하고, `D2CResponseEquipItem.unloaded_ammo_slot`으로 계산 결과를 검산해야 한다. 칸이 어긋나면 `C2DRequestRecentInventoryInfo`로 전체를 다시 받을 것. 빈 칸이 없어 탄약이 폐기되는 갈래의 UI 처리도 함께 필요하다
...
- **`PlayerInventory`의 `*Inventory` 계열 넷은 호출부가 없는 죽은 코드다** — `EquipWeaponFromInventory`·`UnequipWeaponToInventory`·`EquipArmorFromInventory`·`UnequipArmorToInventory`. 살아 있는 것은 `*Slot` 계열뿐이다. 되살린다면 `UnequipWeaponToInventory`의 순서를 먼저 고칠 것 — 탄창을 언로드한 **뒤** 대상 슬롯의 타입을 검사하고 `return false` 하므로, 탄창만 쏟고 버전은 올리지 않은 채 실패한다(`*Slot` 계열은 검사가 언로드보다 앞이라 문제없다)
