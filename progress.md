...
2. **Turret 수치 확정과 첫 실플레이 디버그** — 배치는 붙었으나(2026-09-19 #0) 피격→사망→회수→통보 경로는 **아직 한 번도 실행된 적이 없다**. 남은 것 셋 — ① `HostileNPCs.h`의 잠정 수치 확정(`DEFAULT_MAX_HP` 30000·`ATTACK_DAMAGE` 1200은 AK-47 `baseDamage` 4800과 플레이어 200HP만 보고 넣은 값이고, MIN/MAX_AGGRO는 기본값 4/8을 그대로 쓴다) ② 네 대의 yaw — 현재 전부 0도이고 바라볼 방향이 정해지면 `_tenerifeNpcSpawns`만 고치면 된다 ③ 클라이언트 동반 구현(아래 프리팹 매핑 항목)이 끝난 뒤 전 경로를 그 자리에서 디버그할 것
...
- **2026-09-18 #0·09-19 #0의 변경도 아직 컴파일된 적이 없다** — `DistributeLoot()`과 같은 사정으로, 확인한 것은 `PktId` 짝 전수 대조(`enum.h` ↔ `.proto` ↔ 등록·Make 함수 6개)와 필드명·기존 관용구 일치, 그리고 Turret 배치 쪽의 `ObjectType` 존재·`SpawnDynamicObject` 시그니처·헤더 포함 관계까지다. 문법·타입은 다음 Linux 빌드에서 처음 확인된다
